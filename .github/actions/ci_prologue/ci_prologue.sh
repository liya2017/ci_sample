...
curl -H "Authorization: Bearer $GITHUB_TOKEN" https://api.github.com/repos/$GITHUB_REPOSITORY/collaborators/$GITHUB_ACTOR/permission
  echo "====="
...
# actor_permission=`curl  -H "Authorization: token $ACCESS_GITHUB_TOKEN" https://api.github.com/repos/$GITHUB_REPOSITORY/collaborators/$GITHUB_ACTOR/permission`
echo "actor_permission is" $actor_permission
if [[ $GITHUB_EVENT_NAME == "pull_request" ]];then
  MESSAGE="$PR_COMMONS_BODY"
  echo "GITHUB_REPOSITORY is "$GITHUB_REPOSITORY
  echo "GITHUB_ACTOR is "$GITHUB_ACTOR
  curl -v -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/$GITHUB_REPOSITORY/collaborators/$GITHUB_ACTOR/permission
  echo "====="
  actor_permission=`curl -H "Accept: application/vnd.github.v3+json" https://api.github.com/repos/$GITHUB_REPOSITORY/collaborators/$GITHUB_ACTOR/permission |jq .permission | sed 's/\"//g'`
  echo "actor_permission is" $actor_permission
  if [[ ${LABELS[@]} =~ "ci-trust" ]];then
    ...
    if [[ "$GITHUB_REPOSITORY" == "nervosnetwork/ckb" ]];then
      echo "::set-output name=runner_label::self-hosted-ci-ubuntu-20.04"
    else
      echo "::set-output name=runner_label::ubuntu-20.04"
    fi
